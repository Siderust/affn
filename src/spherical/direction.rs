...
//! let dir = Direction::<WorldFrame>::new_unchecked(45.0 * DEG, 30.0 * DEG);
//! ```
...
impl<F: ReferenceFrame> Direction<F> {
    ...
    /// - `azimuth` in `[0°, 360°)`
    ///
    /// For frame-specific constructors that canonicalize automatically, see the
    /// `new` method generated by `#[derive(ReferenceFrame)]` for spherical frames
    /// (e.g., `Direction::<ICRS>::new(ra, dec)`).
    pub const fn new_unchecked(polar: Degrees, azimuth: Degrees) -> Self {
        Self {
            polar,
            azimuth,
            _frame: PhantomData,
    ...
    /// }
    ///
    /// let dir = Direction::<WorldFrame>::new_unchecked(0.0*DEG, 0.0*DEG);
    ...
    where
        ...
        U: LengthUnit,
    {
        super::Position::new_unchecked(self.polar, self.azimuth, magnitude)
    }
    ...
    where
        ...
        U: LengthUnit,
    {
        super::Position::new_unchecked_with_params(
            center_params,
            self.polar,
            self.azimuth,
            magnitude,
...
mod tests {
    ...
    fn creates_valid_spherical_direction() {
        ...
        let coord = Direction::<TestFrame>::new_unchecked(polar, azimuth);
    ...
    fn displays_coordinate_as_string_correctly() {
        let coord = Direction::<TestFrame>::new_unchecked(Degrees::new(30.0), Degrees::new(60.0));
    ...
    fn position_method_promotes_with_given_radius() {
        let dir = Direction::<TestFrame>::new_unchecked(Degrees::new(-30.0), Degrees::new(120.0));
    ...
    fn angular_separation_identity() {
        let a = Direction::<TestFrame>::new_unchecked(Degrees::new(45.0), Degrees::new(30.0));
    ...
    #[test]
    fn canonicalizes_angle_pair_across_poles() {
        use crate::spherical::canonicalize_polar_azimuth;

        let (polar, azimuth) = canonicalize_polar_azimuth(100.0 * DEG, 30.0 * DEG);
        assert!((polar.value() - 80.0).abs() < EPS);
        assert!((azimuth.value() - 210.0).abs() < EPS);
    ...
    fn roundtrip_spherical_cartesian_direction() {
        let original =
            Direction::<TestFrame>::new_unchecked(Degrees::new(45.0), Degrees::new(30.0));
    ...
    fn roundtrip_at_poles() {
        // North pole
        let north = Direction::<TestFrame>::new_unchecked(Degrees::new(90.0), Degrees::new(0.0));
        let cart_n = north.to_cartesian();
        let recovered_n = Direction::from_cartesian(&cart_n);
        assert!((recovered_n.polar.value() - 90.0).abs() < EPS);

        // South pole
        let south = Direction::<TestFrame>::new_unchecked(Degrees::new(-90.0), Degrees::new(0.0));
    ...
    fn roundtrip_at_azimuth_boundaries() {
        // At azimuth = 0
        let dir0 = Direction::<TestFrame>::new_unchecked(Degrees::new(30.0), Degrees::new(0.0));
        ...
        let dir360 = Direction::<TestFrame>::new_unchecked(Degrees::new(30.0), Degrees::new(359.9));
    ...
    fn uses_raw_construction() {
        let dir = Direction::<TestFrame>::new_unchecked(Degrees::new(10.0), Degrees::new(20.0));
    ...
    fn position_with_params_preserves_center() {
        let dir = Direction::<TestFrame>::new_unchecked(Degrees::new(15.0), Degrees::new(25.0));
